...
int access_ok(int type, const void * addr, unsigned long size)
{
  ...
  //hacer un get frama i veure si es dels compartits i dir ok
  ...
  switch(type)
  {
    case VERIFY_WRITE:
      ...
      if ((addr_ini>=USER_FIRST_PAGE+NUM_PAG_CODE)&&(addr_fin<=USER_FIRST_PAGE+NUM_PAG_CODE+NUM_PAG_DATA)) return 1;
      if ((addr_ini>USER_FIRST_PAGE+NUM_PAG_CODE+NUM_PAG_DATA) && (addr_fin<TOTAL_PAGES)) {
        for (int x = 0; x < 10; ++x) {

          if (get_frame(get_DIR(current()),addr_ini) == 0) return 0; //aquesta pag no esta mapeada, exit
          
        }
      }
    
    //get_frame(get_PT(current()),addr_ini)
    //get frame = 0 implica no ok, no compartida, get frame != 0 ok compartida del shared_vector
    ...
    default:
      ...
      if ((addr_ini>USER_FIRST_PAGE+NUM_PAG_CODE+NUM_PAG_DATA) && (addr_fin<TOTAL_PAGES)) {
        for (int x = 0; x < 10; ++x) {
          if (get_frame(get_PT(current()),addr_ini) == 0) return 0; //aquesta pag no esta mapeada, exit
        }
      }
  }
  return 0;
